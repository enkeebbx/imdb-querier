...
import models.RatingResultResponse
...
class RatingsController @Inject()(
  ...
  /**
    ...
    * @return
    */
  def getRatingsRank(genre: String): Action[AnyContent] = Action.async {
    ratingsQueryService.findTopRatedMoviesByGenre(genre).map {
      case Right(result) =>
